    ...
    static SECTIONS: LazyLock<[Vec<Line<'static>>; PIPELINE_TAB_COUNT]> = LazyLock::new(|| {
        ...
        [
            ...
            vec![
                ...
                Line::from("  Or set break_command in the pipeline TOML instead:"),
                Line::from("  a shell command run after each pass (exit 0 = stop),"),
                Line::from("  a deterministic gate, e.g. \"cargo test\". Mutually"),
                Line::from("  exclusive with break_agent."),
                Line::from(""),
                Line::from(Span::styled(
                    "  Code (Reduce) Blocks",
                    Style::default().fg(Color::Cyan),
                )),
                Line::from("  A block may set command in the pipeline TOML: a"),
                Line::from("  shell command run instead of an agent. Upstream"),
                Line::from("  outputs are piped to stdin; stdout becomes the"),
                Line::from("  block output. Zero tokens — plain-code reduce."),
                Line::from(""),
                Line::from(Span::styled(
                    "  Output Contracts",
                    Style::default().fg(Color::Cyan),
                )),
                Line::from("  A block may set schema in the pipeline TOML: a flat"),
                Line::from("  JSON map of field -> type (string/number/boolean/"),
                Line::from("  array/object/any, \"?\" = optional). The output must"),
                Line::from("  be ONLY a matching JSON object; violations are"),
                Line::from("  rejected and retried (2x), then the block errors."),
                Line::from(""),
                Line::from(Span::styled(
                    "  Scatter Connections",
                    Style::default().fg(Color::Cyan),
